...
var assetsSettings = {
	'js': {
		...
		'path': './public/js/',
		'dataType': 'javascript',
		'files': ['http://code.jquery.com/jquery-latest.js', siteConf.uri + '/socket.io/socket.io.js', // special case since the socket.io module serves its own js
		'jquery.client.js', 'foundation.js', 'd3.min.js'],
		'debug': true,
		'postManipulate': {
			'^': [
			assetHandler.uglifyJsOptimize, function insertSocketIoPort(file, path, index, isLast, callback) {
	...
	'css': {
		...
		'path': './public/css/',
		'dataType': 'css',
		'files': ['reset.css', 'client.css','app.css'],
		'debug': true,
		'postManipulate': {
			'^': [
