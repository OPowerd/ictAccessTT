...
(function() {
    ...
    // Top Level API
    // -------
    Data.VALUE_TYPES = ['string', 'object', 'number', 'boolean', 'date'];
        ...
        _.each(queries, function(query) {
            ...
            _.each(query, function(value, key) {
                ...
                if (operator === "|=") { // one of operator
                    ...
                    switch (operator) {
                    case "!=":
                        condition = !_.isEqual(node[property], value);
                        break;
                    case ">":
  ...
  */
    ...
    _.extend(Data.Node.prototype, _.Events, {
        ...
        set: function(property, key, value) {
            ...
            this._properties[property].set(key, value instanceof Data.Node ? value : new Data.Node({
                value: value
            }));
            return this;
        },
            ...
            this.properties().each(function(property, key) {
                ...
                if ((that.get(key) === undefined || that.get(key) === null) || that.get(key) === "") {
                    ...
                    if (property.unique && !validType(that.get(key), types)) {
                        that.errors.push({
                            property: key,
                            message: "Invalid type for property \"" + property.name + "\""
                        });
                    }

                    // Non unique properties
                    if (!property.unique && !_.all(that.get(key).values(), function(v) {
                        return validType(v, types);
                    })) {
                        that.errors.push({
                            property: key,
                            message: "Invalid value type for property \"" + property.name + "\""
                        });
                    }
                }
                ...
                if (property.validator) {
                    if (!validValue()) {
                        that.errors.push({
                            property: key,
                            message: "Invalid value for property \"" + property.name + "\""
                        });
                    }
                }
            });
    ...
    Data.Graph = _.inherits(Data.Node, {
        ...
        set: function(node) {
            ...
            if (arguments.length === 2) node = _.extend(arguments[1], {
                _id: arguments[0]
            });
        ...
        sync: function(callback) {
            ...
            this.adapter.write(validNodes.toJSON(), function(err, g) {
                ...
                var unsavedNodes = that.invalidNodes().union(that.conflictedNodes()).union(that.rejectedNodes()).length;
    ...
    _.extend(Data.Collection.prototype, {
        ...
        set: function(id, properties) {
            this.g.set(id, _.extend(properties, {
                type: "/type/item"
            }));
        },
